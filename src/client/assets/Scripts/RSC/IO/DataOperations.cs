namespace Assets.RSC.IO
{
	...
	public class DataOperations
	{
		...
		public static int getByte(sbyte[] data, int offset)
		{
		...
		public static int getUnsigned2Bytes(sbyte[] abyte0, int i)
		{
			return ((abyte0[i] & 0xff) << 8) + (abyte0[i + 1] & 0xff);
		}

		public static int getUnsigned4Bytes(sbyte[] abyte0, int i)
		{
			return ((abyte0[i] & 0xff) << 24) + ((abyte0[i + 1] & 0xff) << 16) + ((abyte0[i + 2] & 0xff) << 8) + (abyte0[i + 3] & 0xff);
		}

		public static long getUnsigned8Bytes(sbyte[] abyte0, int i)
		{
			return (((long)getUnsigned4Bytes(abyte0, i) & 0xffffffffL) << 32) + ((long)getUnsigned4Bytes(abyte0, i + 4) & 0xffffffffL);
		}
