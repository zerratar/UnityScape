...
public final class DataConversions {
	...
	/**
	 *	Decodes the clan tag hash into its string representation
	 *	@param clanTag
	 *		The hash to decode
	 *	@return
	...
	public static String hashToClanTag(int clanTag) {
		if(clanTag < 0) {
			return "null";
		}
		String tag = "";
		while(clanTag != 0) {
			int oneBlock = clanTag % 37;
			clanTag = clanTag / 37;
			if(oneBlock == 0) {
				tag = " " + tag;
			} else if(oneBlock < 27) {
				if(oneBlock % 37 == 0) {
					tag = (char)((oneBlock + 65) - 1) + tag;
				} else {
					tag = (char)((oneBlock + 97) - 1) + tag;
				}
			} else {
				tag = (char) ((oneBlock + 48) - 27) + tag;
			}
		}
		return tag;
	}
