...
public class GameImage implements ImageProducer, ImageObserver {
    ...
    public GameImage(int width, int height, int k, Component component) {
        ...
        try {
        	File sFile = new File(mudclient.DATA_DIR + "sprites");
            spriteArchive = new ZipFile(sFile);
        } catch (Exception e) {
            e.printStackTrace();
			System.exit(-1);
        }
        ...
        try {
            ...
            for (int offset = 0; offset < string.length(); offset++)
                ...
                if (string.charAt(offset) == '#' && offset + 4 < string.length() && string.charAt(offset + 4) == '#' && string.substring(offset + 1, offset + 4).equalsIgnoreCase("pmd")) {
                    spriteClip4(x - 12, y - 16, 30, 20, 2339, 65280, 0, 0, false);
                    x += 14;
                    offset += 4;
                } else 
				if (string.charAt(offset) == '#' && offset + 4 < string.length() && string.charAt(offset + 4) == '#' && string.substring(offset + 1, offset + 4).equalsIgnoreCase("cmd")) {
                    spriteClip4(x - 12, y - 16, 30, 20, 2339, 255, 0, 0, false);
                    x += 14;
                    offset += 4;
                } else
				if (string.charAt(offset) == '#' && offset + 4 < string.length() && string.charAt(offset + 4) == '#' && string.substring(offset + 1, offset + 4).equalsIgnoreCase("dev")) {
