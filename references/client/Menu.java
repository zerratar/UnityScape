...
public class Menu {
    ...
    protected void drawSlider(int i, int j, int k, int l, int i1, int j1) { // draws slider
        int k1 = (i + k) - 12;
    ...
    protected void method153(int i, int j, int k, int l, String as[]) {
		...
		protected void method154(int pointer, int x, int y, int width, int height, int textType, String menuListText[], int menuListTextCount, int l1) { // Editing it up...
        int visibleTextLines = height / gameImage.messageFontHeight(textType);
        if (visibleTextLines < menuListTextCount) {
            int j2 = (x + width) - 12;
            int l2 = ((height - 27) * visibleTextLines) / menuListTextCount;
            if (l2 < 6)
                l2 = 6;
            int j3 = ((height - 27 - l2) * l1) / (menuListTextCount - visibleTextLines);
            if (mouseButton == 1 && mouseX >= j2 && mouseX <= j2 + 12) {
                if (mouseY > y && mouseY < y + 12 && l1 > 0)
                    l1--;
                if (mouseY > (y + height) - 12 && mouseY < y + height && l1 < menuListTextCount - visibleTextLines)
                    l1++;
                anIntArray187[pointer] = l1;
            }
            if (mouseButton == 1 && (mouseX >= j2 && mouseX <= j2 + 12 || mouseX >= j2 - 12 && mouseX <= j2 + 24 && aBooleanArray184[pointer])) {
                if (mouseY > y + 12 && mouseY < (y + height) - 12) {
                    aBooleanArray184[pointer] = true;
                    int l3 = mouseY - y - 12 - l2 / 2;
                    l1 = (l3 * menuListTextCount) / (height - 24);
                    if (l1 < 0)
                        l1 = 0;
                    if (l1 > menuListTextCount - visibleTextLines)
                        l1 = menuListTextCount - visibleTextLines;
                    anIntArray187[pointer] = l1;
                }
            } else {
                aBooleanArray184[pointer] = false;
            }
            j3 = ((height - 27 - l2) * l1) / (menuListTextCount - visibleTextLines);
            drawSlider(x, y, width, height, j3, l2); // handles slider
        } else {
            l1 = 0;
            anIntArray187[pointer] = 0;
        }
        anIntArray190[pointer] = -1;
        int k2 = height - visibleTextLines * gameImage.messageFontHeight(textType);
        int i3 = y + (gameImage.messageFontHeight(textType) * 5) / 6 + k2 / 2;
        for (int k3 = l1; k3 < menuListTextCount; k3++) {
            ...
            if (menuObjectColourMask[pointer])
                ...
                i4 = 0;
            if (mouseX >= x + 2 && mouseX <= x + 2 + gameImage.textWidth(menuListText[k3], textType) && mouseY - 2 <= i3 && mouseY - 2 > i3 - gameImage.messageFontHeight(textType)) {
                if (menuObjectColourMask[pointer])
                    i4 = 0x808080;
                else
                    i4 = 0xffffff;
                ...
                if (lastMouseButton == 1) {
                    anIntArray189[pointer] = k3;
                    menuObjectHasAction[pointer] = true;
                }
            }
            if (anIntArray189[pointer] == k3 && redStringColour)
                i4 = 0xff0000;
            gameImage.drawString(menuListText[k3], x + 2, i3, textType, i4);
            i3 += gameImage.messageFontHeight(textType);
            if (i3 >= y + height)
                return;
        }
    }
	
    /*protected void method154(int i, int j, int k, int l, int i1, int j1, String as[], int k1, int l1) { // Editing it up...
        int i2 = i1 / gameImage.messageFontHeight(j1);
        if (i2 < k1) {
            int j2 = (j + l) - 12;
            int l2 = ((i1 - 27) * i2) / k1;
            ...
            j3 = ((i1 - 27 - l2) * l1) / (k1 - i2);
            drawSlider(j, k, l, i1, j3, l2); // handles slider
        } else {
            l1 = 0;
            anIntArray187[i] = 0;
        }
